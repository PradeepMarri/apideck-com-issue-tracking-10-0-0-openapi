...
func CollectionticketcommentsaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CollectionTicketComment
		...
		url := fmt.Sprintf("%s/issue-tracking/collections/%s/tickets/%s/comments%s", cfg.BaseURL, collection_id, ticket_id, queryString)
		...
		var result models.CreateCommentResponse
...
func CreateCollectionticketcommentsaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_issue-tracking_collections_collection_id_tickets_ticket_id_comments",
		mcp.WithDescription("Create Comment"),
		...
		mcp.WithString("body", mcp.Description("Input parameter: Body of the comment")),
		mcp.WithString("created_at", mcp.Description("Input parameter: The date and time when the object was created.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CollectionticketcommentsaddHandler(cfg),
	}
}
